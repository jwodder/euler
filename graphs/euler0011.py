...
grid = [map(int, re.findall(r'\d+', l)) for l in __doc__.splitlines()[5:25]]
...
def sequences():
    ...
    for col in zip(*grid):
        yield col
    size = len(grid)
    for y0 in range(0, size-N+1):
        yield [grid[y0+x][x] for x in range(0, size-y0)]
